...
import frc.robot.subsystems.OldManualShooter;
...
public class RobotContainer {
  ...
  // Subsystem creation
  public final OldPneumaticsSubsystem m_PneumaticsSubsystem = new OldPneumaticsSubsystem();
  private Swerve m_swerve = new Swerve();
  // private HybridShooter m_hybridShooter = new HybridShooter(m_swerve,
  // m_PneumaticsSubsystem);

  private OldIntake m_intake = new OldIntake();
  private OldManualShooter m_manShooter = new OldManualShooter(m_swerve, m_PneumaticsSubsystem);
  ...
  private void configureButtonBindings() {
    ...
    // m_swerve.resetEncoders(), m_swerve));
    ...
    // m_arm.getExtensionEncoder().setPosition(0), m_arm));
     ...
     * m_winch.winchExtend(m_operatorController)));
     * }
    ...
    m_operatorController.y().whileTrue(new RunCommand(() -> m_intake.on(Constants.Intake.kIntakeSpeed), m_intake))
    ...
    m_operatorController.rightBumper()
        .whileTrue(new RunCommand(() -> m_manShooter.shooterAngleUp(Constants.Shooter.shooterAngleSpeed)))
        .or(m_operatorController.leftBumper()
            .whileTrue(new RunCommand(() -> m_manShooter.shooterAngleDown(Constants.Shooter.shooterAngleSpeed))))
        .onFalse(new RunCommand(() -> m_manShooter.angleOff()));

    // slow mode for right bumper, medium slow for left bumper

    // Changed from slow to fast on the first one and changed from medium to fast on
    // the second m_swerve
  ...
  private void addCommandDropdown() {
    ...
    // chooser.addOption("Shoot and Move", new ShootAndMove(m_hybridShooter,
    // m_swerve));

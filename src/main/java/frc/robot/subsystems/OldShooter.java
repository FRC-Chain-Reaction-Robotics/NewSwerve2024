...
public class OldShooter extends SubsystemBase {
     ...
     private OldPneumaticsSubsystem m_PneumaticsSubsystem;
     ...
     private boolean withinYToleranceGround = (Math
               .abs(throughBoreEncoder.getPosition() - yTarget[0]) < Constants.Shooter.yTolerance);
     private boolean withinYToleranceShooter = (Math
               .abs(throughBoreEncoder.getPosition() - yTarget[1]) < Constants.Shooter.yTolerance);

     public OldShooter(Swerve m_Swerve, OldPneumaticsSubsystem m_PneumaticsSubsystem) {
     ...
     public void cherryBomb() {
          if (m_Apriltags.getV() != 0) {
               ...
               boolean withinXTolerance = (m_Apriltags.getX() < Constants.Shooter.xTolerance
                         && m_Apriltags.getX() > -Constants.Shooter.xTolerance);
               boolean withinAreaTolerance = (m_Apriltags.getArea()
                         - Constants.Shooter.targetArea < Constants.Shooter.areaTolerance
                         && m_Apriltags.getArea() - Constants.Shooter.targetArea > Constants.Shooter.areaTolerance);
               shooterTalonFXTwo.set(Constants.Shooter.launchSpeedLimit);
               shooterTalonFXFour.set(Constants.Shooter.launchSpeedLimit);
               while (shooterTalonFXTwo.getVelocity().getValueAsDouble() < 0.6
                         && shooterTalonFXFour.getVelocity().getValueAsDouble() < 0.6 || !Constants.Shooter.onTarget) {
                    ...
                    if (!withinYToleranceShooter) {
                         shooterTalonFXTwo
                                   .set(m_YPidController.calculate(throughBoreEncoder.getPosition(), yTarget[1]));
                         shooterTalonFXFour
                                   .set(m_YPidController.calculate(throughBoreEncoder.getPosition(), yTarget[1]));
                    }
